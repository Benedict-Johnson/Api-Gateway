...
async def main():
    async with httpx.AsyncClient(base_url="http://localhost:8000") as client:
        ...
        for _ in range(5):
            await client.get("/users", headers={"X-API-Key": "secret123"})
            
        # Trigger rate limit
        for _ in range(15):
            await client.get("/slow", headers={"X-API-Key": "secret123"})
            
        # Trigger an error (invalid route)
        await client.get("/notfound", headers={"X-API-Key": "secret123"})
