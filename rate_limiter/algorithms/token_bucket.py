...
class TokenBucketLimiter(RateLimiter):
    ...
    async def allow_request(self, key: str) -> RateLimitResult:
        ...
        if tokens < 1:
            await redis_client.hset(redis_key, mapping={"tokens": tokens, "last": now})
            return RateLimitResult(allowed=False, limit=capacity, remaining=int(tokens))

        tokens -= 1

        await redis_client.hset(redis_key, mapping={"tokens": tokens, "last": now})

        return RateLimitResult(allowed=True, limit=capacity, remaining=int(tokens))
