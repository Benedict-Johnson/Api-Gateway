...
GATEWAY_ACTIVE_REQUESTS = Gauge(
    "gateway_active_requests", "Number of requests currently being processed"
)

# Error Metrics
GATEWAY_4XX_TOTAL = Counter(
    "gateway_4xx_total", "Total number of 4xx responses", ["service"]
)

GATEWAY_5XX_TOTAL = Counter(
    "gateway_5xx_total", "Total number of 5xx responses", ["service"]
)
...
# Cache Metrics
GATEWAY_CACHE_HITS = Counter("gateway_cache_hits_total", "Total number of cache hits")

GATEWAY_CACHE_MISSES = Counter(
    "gateway_cache_misses_total", "Total number of cache misses"
)

# Rate Limiter Metrics
GATEWAY_RATE_LIMIT_HITS = Counter(
    "gateway_rate_limit_hits_total", "Total number of rate limited requests"
)

# Resilience Metrics
GATEWAY_RETRY_TOTAL = Counter(
    "gateway_retry_total", "Total number of retries executed", ["service"]
)

GATEWAY_TIMEOUT_TOTAL = Counter(
    "gateway_timeout_total", "Total number of upstream timeouts", ["service"]
)
