    ...
    async def trigger_shutdown(self):
        ...
        while self.active_requests > 0:
            ...
            if elapsed >= self.config.graceful_timeout:
                logger.warning(f"[SHUTDOWN] Graceful timeout ({self.config.graceful_timeout}s) reached with {self.active_requests} requests still active.")
                break
                
            logger.info(f"[SHUTDOWN] Waiting for {self.active_requests} active requests")
