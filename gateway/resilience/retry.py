...
class RetryPolicy:
    ...
    async def execute(self, client: httpx.AsyncClient, method: str, url: str, headers: dict, params: dict, content: bytes):
        for attempt in range(self.config.attempts):
            try:
                ...
                logger.warning(f"Retry attempt {attempt + 1}/{self.config.attempts} for {url}")
